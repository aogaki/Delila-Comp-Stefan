...
int VMEcrate::daq_unconfigure()
{
    ...
    ptr_mod_run_v.clear();
    std::vector<runFct>().swap(ptr_mod_run_v);

    ptr_mod_checkifreader_v.clear();
    std::vector<checkifreaderFct>().swap(ptr_mod_checkifreader_v);

    ptr_mod_stop_v.clear();
    std::vector<stopFct>().swap(ptr_mod_stop_v);

    ptr_mod_start_v.clear();
    std::vector<startFct>().swap(ptr_mod_start_v);

    ptr_mod_configure_v.clear();
    std::vector<configureFct>().swap(ptr_mod_configure_v);
...
int VMEcrate::read_data_from_detectors()
{
    ...
    for(int iter = 0; iter<nrMods; iter+=2){
        ...
        if((*ptr_mod_checkifreader_v[iter/2])() == 1){
            ...
            (*ptr_mod_run_v[iter/2])(ptr_utilsVMEbltRead, ptr_utilsVMEwrite, ptr_setAddrMod, ptr_setDataW, ptr_utilsVMEirqCheck, modAddr_v[iter/2], 
